...
type ChannelOtherSettings struct {
	...
	OpenAIVideoNewAPIRelay                bool                  `json:"openai_video_newapi_relay,omitempty"`                  // OpenAI 渠道视频任务直通 New API 兼容上游（使用 /v1/video/generations 而非 /v1/videos）
	OpenAIImageNewAPIRelay                bool                  `json:"openai_image_newapi_relay,omitempty"`                  // OpenAI 渠道图片生成直通 New API 兼容上游（使用上游 /v1/images/generations，同步返回）
